...
def generate_payload(jobTitle, productUsed, searchQueries):
    instantPrompt = f"based on the products and product descriptions, as a user with jobTitle: {jobTitle}, productUsed: {productUsed}, and searchQueries: {', '.join(searchQueries)}, recommend 3 products, rank them based on the highest relevancy with percentage. Provide the answer in json array format."
...
# Example usage
jobTitle = "Auditor"
productUsed = "Ideagen Audit & Risk"
searchQueries = ["audit workflow", "evidence management"]

payload = generate_payload(jobTitle, productUsed, searchQueries)
print(payload)
